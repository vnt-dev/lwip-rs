...
pub fn to_socket_addr(ip: &ip_addr_t, port: u16_t) -> SocketAddr {
    SocketAddr::new(to_ip_addr(ip), port)
}

pub fn to_ip_addr(ip: &ip_addr_t) -> IpAddr {
    unsafe {
        match ip.type_ {
            // Ipv4
            0 => IpAddr::V4(ip.u_addr.ip4.addr.to_ne_bytes().into()),
            // Ipv6
            6 => {
                let addr = ip.u_addr.ip6.addr;
